...
public class Student_Controller {
    ...
    public int insert(tbl_Student s){
        int row = 0;
        String sql = "insert into tbl_Student(MaSV,Name,Phone,Email, Address,DOB,Gender,img,Status,Class_ID) values (?,?,?,?,?,?,?,?,?,?)";
        try {
    ...
    public List<tbl_Student> select(String check){
        ...
        try {
            ...
            while (rs.next()) {                
                ...
                LS.add(new tbl_Student(id, masv, name, phone, mail, address, dob, status, class_id, img, gender));
            }
        } catch (SQLException ex) {
    ...
    public boolean update(tbl_Student s){
        boolean check;
        String sql = "update tbl_Student set MaSV = ?, Name = ?, Phone = ?, Email = ?, "
